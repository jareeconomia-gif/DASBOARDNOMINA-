...
function sessionSignature(payload) { return crypto.createHmac('sha256', SESSION_SECRET).update(payload).digest('base64url'); }
...
function currentSession(req) { return verifySessionToken(parseCookies(req).subtec_session); }
function requireAuth(req, res) {
  const session = currentSession(req);
  if (!session) { sendJson(res, 401, { error: 'Sesión requerida' }); return null; }
  return session;
}
...
function readJson(req, maxBytes = 100 * 1024 * 1024) {
  return new Promise((resolve, reject) => {
    ...
    req.on('data', chunk => {
      size += chunk.length;
      if (size > maxBytes) { reject(Object.assign(new Error('Payload demasiado grande'), { status: 413 })); req.destroy(); return; }
      chunks.push(chunk);
    });
    req.on('end', () => {
...
function repairFinancialRecord(raw) {
  const r = { ...(raw || {}) };
  const cost = Number(r.costo_empresa) || 0;
  const pnor = Number(r.pnor) || 0;
  const per = Number(r.percepciones) || 0;
  const prov = Number(r.provisiones) || 0;
  const hasComponents = per > 0 && prov >= 0 && (per + prov) > 0;
  const equalsPnor = cost > 0 && pnor > 0 && Math.abs(cost - pnor) < 0.01;
  const suspicious = r.financial_inferred === true || r.financial_source === 'PNOR' || equalsPnor;

  if (suspicious && hasComponents) {
    r.costo_empresa = per + prov;
    r.financial_inferred = false;
    r.financial_missing = false;
    r.financial_source = 'PERCEPCIONES_MAS_PROVISIONES_REPARADO';
    r.financial_repaired = true;
  } else if (suspicious && !hasComponents) {
    r.costo_empresa = 0;
    r.financial_missing = true;
    r.financial_source = 'HC_ONLY_REQUIERE_COSTO_EMPRESA';
    r.financial_repaired = true;
  }
  return r;
}
function normalizeState(raw) {
  return {
    records: Array.isArray(raw?.records) ? raw.records.map(repairFinancialRecord) : [],
    filesMeta: Array.isArray(raw?.filesMeta) ? raw.filesMeta : [],
    updatedAt: raw?.updatedAt || new Date(0).toISOString()
  };
}
function recordKey(r) {
  return [r?.payroll, r?.period, r?.employee_id, r?.ceco, r?.vessel || '', r?.activity || ''].map(v => String(v ?? '')).join('|');
}
function mergeRecords(base, incoming) {
  const map = new Map();
  (base || []).map(repairFinancialRecord).forEach(r => map.set(recordKey(r), r));
  (incoming || []).map(repairFinancialRecord).forEach(r => map.set(recordKey(r), r));
  return [...map.values()];
}
function fileMetaKey(x) { return String(x?.name || '').trim().toLowerCase(); }
function mergeFilesMeta(base, incoming) {
  const map = new Map();
  (base || []).forEach(x => map.set(fileMetaKey(x), x));
  (incoming || []).forEach(x => { const k = fileMetaKey(x); if (k) map.set(k, { ...(map.get(k) || {}), ...x }); });
  return [...map.values()];
}
...
async function initStorage() {
  ...
  try {
    ...
    await pool.query(`CREATE TABLE IF NOT EXISTS dashboard_state (id INTEGER PRIMARY KEY, records JSONB NOT NULL DEFAULT '[]'::jsonb, files_meta JSONB NOT NULL DEFAULT '[]'::jsonb, updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW())`);
...
async function getFileState() {
  try { return normalizeState(JSON.parse(await fs.promises.readFile(FALLBACK_STATE, 'utf8'))); }
  catch (_) { return { records: [], filesMeta: [], updatedAt: new Date(0).toISOString() }; }
}
async function saveFileState(state) {
  const next = normalizeState({ ...state, updatedAt: new Date().toISOString() });
  await fs.promises.writeFile(FALLBACK_STATE, JSON.stringify(next), 'utf8'); return next;
}
...
async function replaceState(state) {
  ...
  const { rows } = await pool.query(`UPDATE dashboard_state SET records=$1::jsonb, files_meta=$2::jsonb, updated_at=NOW() WHERE id=1 RETURNING records, files_meta, updated_at`, [JSON.stringify(clean.records), JSON.stringify(clean.filesMeta)]);
  const row = rows[0]; return normalizeState({ records: row.records, filesMeta: row.files_meta, updatedAt: row.updated_at?.toISOString?.() || row.updated_at });
}
async function mergeState(incoming) {
  ...
  try {
    ...
    const records = mergeRecords(current.records || [], cleanIncoming.records || []);
    const filesMeta = mergeFilesMeta(current.files_meta || [], cleanIncoming.filesMeta || []);
    const result = await client.query(`UPDATE dashboard_state SET records=$1::jsonb, files_meta=$2::jsonb, updated_at=NOW() WHERE id=1 RETURNING records, files_meta, updated_at`, [JSON.stringify(records), JSON.stringify(filesMeta)]);
    await client.query('COMMIT');
    const row = result.rows[0]; return normalizeState({ records: row.records, filesMeta: row.files_meta, updatedAt: row.updated_at?.toISOString?.() || row.updated_at });
  } catch (e) { await client.query('ROLLBACK'); throw e; }
  finally { client.release(); }
}
async function clearState() { return replaceState({ records: [], filesMeta: [] }); }
...
async function loadPersisted(){try{const data=await fetchSharedState();return Array.isArray(data.records)?data.records:null}catch(e){if(e.status!==401){console.warn(e);setSharedSyncStatus("error","Sin conexión a base")};return null}}
async function loadPersistedFilesMeta(){return Array.isArray(sharedFilesMetaCache)?sharedFilesMetaCache:null}
async function clearPersisted(){sharedSyncBusy=true;setSharedSyncStatus("syncing","Borrando base...");try{const data=await sharedApi("/api/state",{method:"DELETE"});sharedStateUpdatedAt=data.updatedAt||null;sharedFilesMetaCache=[];setSharedSyncStatus("","Base compartida · vacía");return true}catch(e){setSharedSyncStatus("error","No se pudo borrar");if(e.status===401)showLogin();else toast("No se pudo borrar la base compartida: "+e.message);return false}finally{sharedSyncBusy=false}}
async function refreshSharedState({force=false,notify=false}={}){if(sharedSyncBusy||document.hidden)return;sharedSyncBusy=true;try{const data=await sharedApi("/api/state");const changed=force||(!sharedStateUpdatedAt&&data.updatedAt)||(data.updatedAt&&data.updatedAt!==sharedStateUpdatedAt);if(changed){records=Array.isArray(data.records)?data.records.map(migrateRecoveredRecord):[];loadedFilesMeta=Array.isArray(data.filesMeta)?data.filesMeta:[];sharedFilesMetaCache=loadedFilesMeta;sharedStateUpdatedAt=data.updatedAt||null;try{renderAll(true)}catch(e){console.error(e)}if(notify)toast("Base compartida actualizada: "+num(loadedFileCount())+" archivos · "+num(records.length)+" registros.")}setSharedSyncStatus("",records.length?"Base compartida · sincronizada":"Base compartida · vacía")}catch(e){if(e.status===401)showLogin();else setSharedSyncStatus("error","Sin conexión a base")}finally{sharedSyncBusy=false}}
...
function transformIndex(source) {
  let html = source;
  html = html.replace('V93 · CARGA HISTÓRICA ROBUSTA', 'V100 · COSTO EMPRESA CORREGIDO');
  html = html.replace('V99 · BASE COMPARTIDA', 'V100 · COSTO EMPRESA CORREGIDO');
  ...
  html = html.replace(
    'const financialAvailable=C.costo_empresa>=0||(C.percepciones>=0&&C.provisiones>=0)||C.pnor>=0;',
    'const reliableCostColumn=C.costo_empresa>=0&&!C.__financial_inferred;const financialAvailable=reliableCostColumn||(C.percepciones>=0&&C.provisiones>=0);'
  );
  html = html.replace(
    'const perce=toNum(get("percepciones")),prov=toNum(get("provisiones")),pnorVal=toNum(get("pnor")),descansoTrab=toNum(get("descanso_trabajado")),cost=C.costo_empresa>=0?toNum(get("costo_empresa")):(perce||prov?perce+prov:pnorVal);',
    'const perce=toNum(get("percepciones")),prov=toNum(get("provisiones")),pnorVal=toNum(get("pnor")),descansoTrab=toNum(get("descanso_trabajado"));const cost=reliableCostColumn?toNum(get("costo_empresa")):(C.percepciones>=0&&C.provisiones>=0?perce+prov:0);'
  );
  html = html.replace(
    'const rec={financial_inferred:!!C.__financial_inferred,financial_missing:!financialAvailable,payroll,',
    'const rec={financial_inferred:!!C.__financial_inferred,financial_missing:!financialAvailable,financial_source:reliableCostColumn?"COSTO_EMPRESA":(C.percepciones>=0&&C.provisiones>=0?"PERCEPCIONES_MAS_PROVISIONES":"HC_ONLY"),payroll,'
  );
...
function sendIndex(res, filePath) {
  fs.readFile(filePath, 'utf8', (err, source) => {
    if (err) { res.writeHead(404, { 'Content-Type': 'text/plain; charset=utf-8' }); res.end('Archivo no encontrado'); return; }
    const html = transformIndex(source);
    res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': 'no-cache, no-store, must-revalidate' }); res.end(html);
  });
}
function sendFile(res, filePath) {
  ...
  fs.readFile(filePath, (err, data) => {
    if (err) { res.writeHead(404, { 'Content-Type': 'text/plain; charset=utf-8' }); res.end('Archivo no encontrado'); return; }
    const ext = path.extname(filePath).toLowerCase(); res.writeHead(200, { 'Content-Type': mimeTypes[ext] || 'application/octet-stream', 'Cache-Control': 'public, max-age=86400' }); res.end(data);
  });
}
...
async function handleApi(req, res, urlPath) {
  if (urlPath === '/api/login' && req.method === 'POST') {
    ...
    if (String(body.username || '') !== LOGIN_USER || String(body.password || '') !== LOGIN_PASS) { sendJson(res, 401, { error: 'Credenciales incorrectas' }); return true; }
    const token = createSessionToken(LOGIN_USER); sendJson(res, 200, { ok: true, username: LOGIN_USER }, { 'Set-Cookie': cookieHeader(req, token) }); return true;
  }
  if (urlPath === '/api/logout' && req.method === 'POST') { sendJson(res, 200, { ok: true }, { 'Set-Cookie': cookieHeader(req, '', true) }); return true; }
  if (urlPath === '/api/session' && req.method === 'GET') { const session = currentSession(req); sendJson(res, 200, { authenticated: !!session, username: session?.username || null }); return true; }
  ...
  sendJson(res, 404, { error: 'API no encontrada' }); return true;
}
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (urlPath === '/' || urlPath === '/index.html') { const indexPath = existingIndex(); if (!indexPath) { res.writeHead(500, { 'Content-Type': 'text/plain; charset=utf-8' }); res.end('No se encontró index.html'); return; } sendIndex(res, indexPath); return; }
    ...
    const safeCandidate = candidates.find(filePath => {
      const inPublic = filePath.startsWith(PUBLIC_DIR + path.sep), inRoot = filePath.startsWith(ROOT_DIR + path.sep);
    ...
    const indexPath = existingIndex(); if (indexPath) { sendIndex(res, indexPath); return; }
    res.writeHead(404, { 'Content-Type': 'text/plain; charset=utf-8' }); res.end('Recurso no encontrado');
  } catch (e) { console.error('Error request', req.method, urlPath, e); sendJson(res, e.status || 500, { error: e.message || 'Error interno del servidor' }); }
});

initStorage().finally(() => {
  server.listen(PORT, '0.0.0.0', () => { console.log(`SUBTEC Dashboard ejecutándose en puerto ${PORT}`); console.log(`Storage: ${storageMode}`); });
});
